...
- **Board:** GitHub Issues is the single source of truth; one issue per slice card, epic labels, `blocked` as a label. An optional Projects view is disposable UI. No parallel backlog file in the tree (an in-repo plan doc used as a board demonstrably drifted in this project's own history).
