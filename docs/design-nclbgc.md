...
| N2 | Unresolved license numbers | Recorded as a flagged `unresolved` record (with the searched number and a reason), counted in the run report; never dropped. Satisfies "resolve or flag it." |
| N3 | The join key vs the `L.` prefix | The NCLBGC detail shows the number with an `L.` prefix (for example `L.68764`) while eVP and our search use the digits (`68764`). **Join on the digit-normalized form**; store both verbatim (the displayed `L.`-form as the license's own field, the digits as the join key). Report-don't-coerce: neither stored value is mutated. |
