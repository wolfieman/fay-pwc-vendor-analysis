...
| N2 | Unresolved license numbers | Slice 2 matches by **license number only** and records a flagged `unresolved` record for any miss (searched number, reason), counted in the run report; never silently dropped. The **name-based reconciliation** of those misses, and the decision to exclude the unusable rows, are **slice 3 (validation)** work — the documented home of the referential check and reconciliation metric (orphan count, match rate). Slice 2 only flags; it does not search by name. |
| N3 | NCLBGC account-number type-sigils (`L.` license, `Q.` qualifier) | NCLBGC prefixes every account number with a uniform single-letter type-sigil and period — `L.68764` (license), `Q.25342` (qualifier) — carrying no discriminating information in context (verified: only `L.` and `Q.` occur). **Strip the sigil at processed-write** on both the license number and the qualifier number to recover the bare digits, logged as an audited correction; the raw capture keeps the sigil verbatim. This scopes REQ-09's "never prefix-strip" to *meaningful* prefixes (out-of-state, multi-license) which still surface as violations; a uniform type-sigil is normalization, not silent coercion. The license's digit form is the join key back to eVP. |
